...
def add_text_file_info(hd, data, offset, fmt, header):
	off = offset
	if header.content_index == 0 or header.linked_text_offset == 0:
		off += 4
...
def handle_page(page, data, offset, parent, fmt, version, obfctx, index, master):
	...
	if settings_blocks_count == 1:
		off = _add_settings('Page settings', off)
                ...
                # some times two pages have settings with 2 blocks 
	...
	for i in range(0, 2*settings_blocks_count + 2 + (1 if fmt() == LITTLE_ENDIAN else 0)):
                # for each blocks in range(settings_blocks_count+1): 'Data%d'%(2*i) often empty ie sz=4, 'Data%d'%(2*i+1) often empty ie sz=0
